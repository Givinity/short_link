...
class TestShortenUrl:
    ...
    async def test_shorten_url_success(self, async_client):
        ...
        with patch("api.routers.tasks.task_service") as mock_service:
            mock_service.shorten = AsyncMock(return_value="abc123")
...
class TestGetHome:
    ...
    async def test_get_home_with_tasks(self, async_client, mock_task):
        ...
        with patch("api.routers.tasks.task_service") as mock_service:
            mock_service.get_all = AsyncMock(return_value=[mock_task])
...
class TestRedirect:
    ...
    async def test_redirect_success(self, async_client, mock_task_response_get):
        """Успешный редирект."""
        with patch("api.routers.redirect.task_service") as mock_service:
            mock_service.get_by_short_link = AsyncMock(
                return_value=mock_task_response_get
            )
    ...
    async def test_redirect_not_found(self, async_client):
        """Ссылка не найдена."""
        with patch("api.routers.redirect.task_service") as mock_service:
            mock_service.get_by_short_link = AsyncMock(return_value=None)
    ...
    async def test_redirect_adds_https(self, async_client):
        ...
        with patch("api.routers.redirect.task_service") as mock_service:
            mock_service.get_by_short_link = AsyncMock(return_value=mock_response)
